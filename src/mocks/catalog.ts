import type { Category, Product, Paginated } from '@/types/catalog';
...
export const products: Product[] = [
  {
    ...
    attributes: {
      ...
      description: 'Підручник з математики для 1 класу. Сучасні методики навчання, яскраві ілюстрації, практичні завдання.',
  ...
  },
  {
    ...
    attributes: {
      ...
      description: 'Підручник з української мови для 2 класу. Вправи, диктанти та завдання для розвитку мовлення.',
  ...
  for (let cls = 1; cls <= 6; cls++) {
    for (const s of subjects) {
      ...
      for (let v = 1; v <= 3; v++) {
        ...
        const categoryIds = ['c1', s.catId, cls === 1 ? 'c2' : cls === 2 ? 'c3' : undefined].filter(Boolean) as string[];
        products.push({
          id,
          title,
          slug,
          ...
          attributes: {
            ...
            rating: { value: 4 + ((cls + v) % 10) / 10, count: 2 + ((cls * v) % 25) },
          },
          categoryIds,
        });
      }
...
export function listProducts(filters: ProductFilters = {}): Paginated<Product> {
  const { q, categoryId, page = 1, perPage = 12, priceMin, priceMax, inStock, onSale, sort = 'newest' } = filters;
  let list = products.slice();
  if (q) {
    const qw = q.toLowerCase();
    list = list.filter(
      (p) => p.title.toLowerCase().includes(qw) || p.attributes.author?.toLowerCase().includes(qw) || p.attributes.publisher?.toLowerCase().includes(qw),
    );
  }
  if (categoryId) {
