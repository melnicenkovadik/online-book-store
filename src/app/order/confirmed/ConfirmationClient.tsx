...
export default function ConfirmationClient() {
  ...
  React.useEffect(() => {
    ...
    fetch(`/api/orders/${number}`)
      ...
      .catch(
        (e: unknown) =>
          !ignore && setError(e instanceof Error ? e.message : "Error"),
      )
  ...
  return (
    ...
    >
      ...
      {number ? (
        <p>
          Номер вашого замовлення: <strong>{number}</strong>
        </p>
      ) : (
          ...
          {!!summary && (
            <div style={{ display: "grid", gap: 6 }}>
              ...
              <div>
                ...
                {summary.totals.grand.toFixed(2)} грн
              </div>
              <div>
              ...
              <div style={{ fontSize: 12, color: "#6b7280", marginTop: 6 }}>
                Будь ласка, вкажіть номер замовлення у призначенні платежу.
              </div>
            </div>
          )}
        </section>
      ...
      <div style={{ marginTop: 16 }}>
        <Link href="/catalog">
          <Button variant="primary">Повернутись до каталогу</Button>
        </Link>
      </div>
    </div>
  );
