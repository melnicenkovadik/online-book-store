...
export default function CheckoutPage() {
  ...
  const [provider, setProvider] = React.useState<"cod" | "fondy" | "liqpay">(
    "cod",
  );
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const payload: {
        ...
        payment: { provider: "cod" | "fondy" | "liqpay" };
      } = {
      ...
      if (cityRef) payload.delivery.cityRef = cityRef;
      if (warehouseRef) payload.delivery.warehouseRef = warehouseRef;
            ...
            <fieldset>
              ...
              <div>
                <label>
                  <input
                    type="radio"
                    checked={carrier === "nova"}
                    onChange={() => setCarrier("nova")}
                  />{" "}
                  Нова Пошта
                </label>
            ...
            <fieldset>
              ...
              <div>
                ...
                <label style={{ marginLeft: 16 }}>
                  <input
                    type="radio"
                    checked={provider === "fondy"}
                    onChange={() => setProvider("fondy")}
                  />{" "}
                  Банківська карта (Fondy)
                </label>
                <label style={{ marginLeft: 16 }}>
                  <input
                    type="radio"
                    checked={provider === "liqpay"}
                    onChange={() => setProvider("liqpay")}
                  />{" "}
                  LiqPay
                </label>
              </div>
