 ...
 */
function shouldExclude(name, _isDirectory = false) {
...
function createFileTree(dirPath, prefix = "", isLast = true, depth = 0) {
  ...
  try {
    ...
    console.warn(
      `Не удалось прочитать директорию ${dirPath}: ${error.message}`,
    );
  }
      ...
      if (item.isDirectory()) {
        ...
        try {
          ...
          if (stats.size <= CONFIG.maxFileSize) {
            ...
            console.warn(
              `Файл ${itemPath} пропущен (размер ${stats.size} байт > ${CONFIG.maxFileSize})`,
            );
          }
        } catch (error) {
          console.warn(
            `Не удалось получить информацию о файле ${itemPath}: ${error.message}`,
          );
        }
      }
    }
...
function createSnapshot(sourceDir, outputFile) {
  ...
  files.sort().forEach((filePath, index) => {
    ...
    console.log(
      `📄 Обработка [${index + 1}/${files.length}]: ${normalizedPath}`,
    );
